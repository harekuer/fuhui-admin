...
(function () {
    ...
    RemoteImage.prototype = {
        ...
        getInsertList: function () {
            ...
            if(data['img_original']) {
                return [{
                    src: data['img_original'],
                    _src: data['img_original'],
                    width: data['width'] || '',
                    height: data['height'] || '',
                    border: data['border'] || '',
        ...
        getInsertListLen: function () {
            var i, lis = this.list.children, list = [], align = getAlign();
            var j = 0,
                num = document.getElementById('choosedNum');
            for (i = 0; i < lis.length; i++) {
                if (domUtils.hasClass(lis[i], 'selected')) {
                    j++
                }
            }
            num.innerHTML = '已选中' + j + '文件'
        }
    };
    ...
    UploadImage.prototype = {
        ...
        initUploader: function () {
            ...
            // else if (!editor.getOpt('multiImageActionName')) {
            ...
            uploader.on('uploadSuccess', function (file, ret) {
                ...
                try {
                    ...
                    if (json.state == 'SUCCESS' || json.code == 200) {
        ...
        getQueueCount: function () {
            ...
            for (i = 0; file = files[i++]; ) {
                ...
                if (status == 'error' || status == 'queued' || status == 'uploading' || status == 'progress') readyFile++;
            }
            return readyFile;
        },
        destroy: function () {
    ...
    OnlineImage.prototype = {
        ...
        initEvents: function () {
            ...
            domUtils.on(this.container, 'click', function (e) {
                ...
                var num = document.getElementById('choosedNum');
                ...
                var list = _this.getInsertList();
                num.innerHTML = '已选中' + list.length + '个文件'
            });

            /* 全选图片 */
            domUtils.on($G('checkAll'), 'click', function (e) {
                var target = e.target || e.srcElement,
                    list = _this.list.children;
                if(target.checked == true){ //全选中
                  for (i = 0; i < list.length -1; i++) {
                    if (!domUtils.hasClass(list[i], 'selected')) {
                        domUtils.addClass(list[i], 'selected');
                    }
                  }
                } else {
                  for (i = 0; i < list.length -1; i++) {//取消选中
                    if (domUtils.hasClass(list[i], 'selected')) {
                        domUtils.removeClasses(list[i], 'selected');
                    }
                  }
                }
                _this.getInsertListLen();
            });

            /* 删除图片 */
            domUtils.on($G('delectChoosed'), 'click', function (e) {
              var list = _this.list.children,
                  newList = [];
                  for (i = 0; i < list.length; i++) {
                    if (domUtils.hasClass(list[i], 'selected')) {
                        var img = list[i].firstChild;
                        newList.push(img.getAttribute('_src'))
                    }
                  }
              if(newList.length > 0) {
                $('#des_dialog').fadeIn();
              } else {
                $('#tip_dialog').find('.con_title').text('请至少选择一张图片！')
                $('#tip_dialog').fadeIn();
                //定时执行，8秒后执行
                var t1=window.setInterval($('#tip_dialog').fadeOut(), 1000 * 8);
                //去掉定时器的方法
                window.clearInterval(t1);
              }
            });

            $('#des_dialog').find('.confirm_yes').on('click', function (e) {
                _this.deleteImageData();
            });
            $('#des_dialog').find('.confirm_no').on('click', function (e) {
                $('#des_dialog').fadeOut();
            });

            /* 复制链接 */
            domUtils.on($G('copyLink'), 'click', function (e) {
              var target = $G('copyValue'),
                  list = _this.list.children,
                  newList = [];
                  for (i = 0; i < list.length; i++) {
                    if (domUtils.hasClass(list[i], 'selected')) {
                  ...
                  //target.value = newList.join(',')
                  //target.setAttribute('data-link', newList.join(','));
                  //_this.initCopy("copyLink");
                  if(newList.length > 0) {
                    ...
                    if (!bool) {
                        ...
                        inputEle.setAttribute('value', newList.join(','))
                        inputEle.setAttribute('readonly', 'readonly')
                        inputEle.select()
                        document.execCommand('copy')
                        document.body.removeChild(inputEle)

                        $('#tip_dialog').find('.con_title').text('复制成功！')
                        $('#tip_dialog').fadeIn();
                        //定时执行，8秒后执行
                        var t1=window.setInterval($('#tip_dialog').hide(), 1000 * 8);
                        //去掉定时器的方法
                        window.clearInterval(t1);
                    }
            ...
            /* 点击搜索按钮 */
            domUtils.on($G('searchSubmit'), 'click', function(){
                var key = $G('searchPic').value;
                var list = _this.listData,
                    domList = _this.list.children,
                    newList = [];
                if(key && key != '') {
                    for (i = 0; i < list.length; i++) {
                      if(list[i].name.toLowerCase().indexOf(key.toLowerCase()) >= 0) {
                        newList.push(list[i])
                      }
                    }
                } else {
                  newList = _this.listData;
                }
                for (i = 0; i < domList.length; i++) {
                  if(!domUtils.hasClass(domList[i], 'clearFloat')) {
                    domList[i].parentNode.removeChild(domList[i])
                    i--;
                  }
                }
                _this.pushData(newList);
            });

            /* 搜索框回车键搜索 */
            domUtils.on($G('searchPic'), 'keydown', function(e){
                var keyCode = e.keyCode || e.which;
                if (keyCode == 13) {
                    $G('searchSubmit').click();
                }
            });
        },
        /* 初始化复制事件 */
        initCopy: function (id) {
              var clip = new ZeroClipboard($G(id), {
                  moviePath: '../../third-party/zeroclipboard/ZeroClipboard.swf',
              });
              clip.on( 'aftercopy', function(client, args) {
                  alert('复制成功, 您可以到其他地方粘贴图片路径了！');
              });
        },
        /* 初始化第一次的数据 */
        initData: function () {
                    ...
                    'onsuccess': function (r) {
                        try {
                            ...
                            if (json.state == 'SUCCESS' || json.code == 200) {
                                ...
                                _this.listIndex = parseInt(json.data.length);
        ...
        pushData: function (list) {
            ...
            for (i = 0; i < list.length; i++) {
                if(list[i] && list[i].img_thumb) {
                    item = document.createElement('li');
                    img = document.createElement('img');
                    icon = document.createElement('span');
                    size = document.createElement('div');
                    title = document.createElement('p');
                    size.innerText = list[i].img_desc;
                    title.innerText = list[i].name;

                    img.width = 113;
                    img.setAttribute('src', urlPrefix + list[i].img_thumb + (list[i].img_thumb.indexOf('?') == -1 ? '?noCache=':'&noCache=') + (+new Date()).toString(36) );
                    img.setAttribute('_src', urlPrefix + list[i].img_thumb);
                    img.setAttribute('img_id', list[i].img_id);
